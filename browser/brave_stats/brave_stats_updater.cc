...
GURL GetUpdateURL(
  ...
  update_url =
      net::AppendQueryParameter(update_url, "uncertainFuture",
                                stats_updater_params.GetUncertainFutureParam());
  return update_url;
}
...
void BraveStatsUpdater::SendServerPing() {
  ...
  auto stats_updater_params =
      std::make_unique<brave_stats::BraveStatsUpdaterParams>(
          pref_service_, profile_pref_service, is_uncertain_future_);
